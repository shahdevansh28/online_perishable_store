...
<c:forEach var="product" items="${products}">
    <form action="/seeProduct" class="product" method="GET">
        <input type="hidden" name="productid" value="${product.productid}" />
        
        <button>
            <image src="${product.productImage}" class="productImage" alt="${product.productName}"></image>
            <!-- <h2>${product.productid}</h2> -->
            <span class="prodName">${product.productName}</span>
            <p class="prodDesc"><span class="prodPrice">${product.productPrice}</span>.00 Rs. ${product.productDescription}</p>
        </button>
    </form>
</c:forEach>
<style>
